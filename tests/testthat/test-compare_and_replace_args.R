...
testthat::test_that("compare_and_replace_args uses explicit arguments", {
  ...
  result <- saros.contents:::compare_and_replace_args(call, defaults_env = env$sarosmake_mock_defaults,
...
testthat::test_that("compare_and_replace_args uses defaults_env if argument not explicit", {
  sarosmake_mock <- function(digits = 1, data_label = "count", showNA = "never") {}
  env <- new.env()
...
testthat::test_that("compare_and_replace_args uses function argument formals if neither explicit nor defaults_env", {
  sarosmake_mock <- function(digits = 1, data_label = "count", showNA = "never") {}
  env <- new.env()
  env$sarosmake_mock_defaults <- list()
  env$ignore_args <- character(0)
...
testthat::test_that("compare_and_replace_args ignores arguments in ignore_args", {
  sarosmake_mock <- function(digits = 1, data_label = "count", showNA = "never") {}
  env <- new.env()
