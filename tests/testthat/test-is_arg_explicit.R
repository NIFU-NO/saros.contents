testthat::test_that("is_arg_explicit detects explicitly passed arguments", {
  test_func <- function(a = 1, b = 2, c = 3) {
    call <- match.call()
    list(
      a_explicit = saros.contents:::is_arg_explicit("a", call),
      b_explicit = saros.contents:::is_arg_explicit("b", call),
      c_explicit = saros.contents:::is_arg_explicit("c", call)
    )
  }
...
testthat::test_that("get_argument_value returns explicitly passed arguments", {
  test_func <- function(a = 1, b = 2, c = 3) {
    ...
    list(
      a_value = saros.contents:::get_argument_value("a", call, defaults_env, formals_list),
      b_value = saros.contents:::get_argument_value("b", call, defaults_env, formals_list),
      c_value = saros.contents:::get_argument_value("c", call, defaults_env, formals_list)
    )
  }
